...
from vdmarl.environments import task_config_registry, TaskClass
from vdmarl.environments.common import _type_check_task_config
from vdmarl.experiment import Experiment, ExperimentConfig
from vdmarl.models import model_config_registry
from vdmarl.models.common import ModelConfig, parse_model_config, SequenceModelConfig
...
def load_experiment_from_hydra(
    ...
    """Creates an :class:`~vdmarl.experiment.Experiment` from hydra config.
    ...
    Returns:
        :class:`~vdmarl.experiment.Experiment`
...
def load_task_config_from_hydra(cfg: DictConfig, task_name: str) -> TaskClass:
    """Returns a :class:`~vdmarl.environments.Task` from hydra config.
    ...
    Returns:
        :class:`~vdmarl.environments.Task`
...
def load_experiment_config_from_hydra(cfg: DictConfig) -> ExperimentConfig:
    """Returns a :class:`~vdmarl.experiment.ExperimentConfig` from hydra config.

    Args:
        cfg (DictConfig): the experiment config dictionary from hydra

    Returns:
        :class:`~vdmarl.experiment.ExperimentConfig`
...
def load_algorithm_config_from_hydra(cfg: DictConfig) -> AlgorithmConfig:
    """Returns a :class:`~vdmarl.algorithms.AlgorithmConfig` from hydra config.

    Args:
        cfg (DictConfig): the algorithm config dictionary from hydra

    Returns:
        :class:`~vdmarl.algorithms.AlgorithmConfig`
...
def load_model_config_from_hydra(cfg: DictConfig) -> ModelConfig:
    """Returns a :class:`~vdmarl.models.ModelConfig` from hydra config.

    Args:
        cfg (DictConfig): the model config dictionary from hydra

    Returns:
        :class:`~vdmarl.models.ModelConfig`
