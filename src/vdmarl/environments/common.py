...
class TaskClass(abc.ABC):
    ...
    def env_name() -> str:
        """
        The name of the environment in the ``vdmarl/conf/task`` folder
        """
        raise NotImplementedError
...
class Task(Enum):
    ...
    Tasks configs are loaded from ``vdmarl/conf/task``.
    """
    ...
    def get_task(self, config: Optional[Dict[str, Any]] = None) -> TaskClass:
        ...
        If no config is given, it will be loaded from ``vdmarl/conf/task/self.env_name()/self.name`` using :meth:`Task.get_from_yaml`.
    ...
    def get_from_yaml(self, path: Optional[str] = None) -> TaskClass:
        ...
        Args:
            path (str, optional): The full path of the yaml file to load from. If None, it will default to
                ``vdmarl/conf/task/self.env_name()/self.name``
