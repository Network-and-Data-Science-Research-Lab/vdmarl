...
from vdmarl.environments import Task, TaskClass
from vdmarl.experiment.callback import Callback, CallbackNotifier
from vdmarl.experiment.logger import Logger
from vdmarl.models import GnnConfig, SequenceModelConfig
from vdmarl.models.common import ModelConfig
from vdmarl.utils import (
    _add_rnn_transforms,
    _read_yaml_config,
    local_seed,
    seed_everything,
...
class ExperimentConfig:
    ...
    To know their meaning, please check out the descriptions in ``vdmarl/conf/experiment/base_experiment.yaml``
    """
    ...
    def get_from_yaml(path: Optional[str] = None):
        ...
        Args:
            path (str, optional): The full path of the yaml file to load from.
                ...
                ``vdmarl/conf/experiment/base_experiment.yaml``

        Returns:
            the loaded :class:`~vdmarl.experiment.ExperimentConfig`
        """
        if path is None:
            yaml_path = (
...
class Experiment(CallbackNotifier):
    """
    Main experiment class in VDMARL.
    ...
    def __init__(
        ...
        if isinstance(task, Task):
            warnings.warn(
                ...
                "If you do not do this, vdmarl will load the default task config from yaml."
            )
            task = task.get_task()
        self.task = task
    ...
    def _setup_task(self):
        ...
        # Add rnn transforms here so they do not show in the vdmarl specs
